...
const Tabs = (props: TabProps) => {
  ...
  let footerStartControl: JSX.Element | null = null;
  let footerEndControl: JSX.Element | null = null;
  if (props.expandedEntry) {
    footerStartControl = (
...
/** There are two tab tabs in the {@see WhatsNewDashboard}, styled like a Switch. */
const SwitchTab = (props: SwitchTabProps) => {
  ...
  return (
    <div
      {...tabProps}
      ref={tabRef}
      className={`${styles.switchTab} ${isSelected ? styles.isSelected : ""}`}
    >
      {props.item.rendered}
    </div>
  );
