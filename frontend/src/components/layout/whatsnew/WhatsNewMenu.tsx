...
export const WhatsNewMenu = (props: Props) => {
  ...
  const newEntries = entries.filter((entry) => {
    const entryDate = new Date(Date.UTC(0, 0, 0));
    entryDate.setUTCFullYear(entry.announcementDate.year);
    entryDate.setUTCMonth(entry.announcementDate.month - 1);
    entryDate.setUTCDate(entry.announcementDate.date);
    ...
    <>
      <button
        ...
        className={`${styles.trigger} ${
          triggerState.isOpen ? styles.isOpen : ""
        }`}
      >
...
const entriesDescByDateSorter: Parameters<Array<WhatsNewEntry>["sort"]>[0] = (
  ...
  const dateANr =
    ...
    entryA.announcementDate.date / 10000;
  const dateBNr =
    entryB.announcementDate.year +
    entryB.announcementDate.month / 100 +
    entryB.announcementDate.date / 10000;
